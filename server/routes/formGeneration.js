...
const { storeSource, deleteSource } = require('../services/extractionStorage')
...
router.post('/', limiter, async (req, res, next) => {
  ...
  try {
    ...
    try {
      ...
      console.error('[document-form-generator] staging failed:', error.message)
      return sendError(res, 'The document could not be stored for processing.', 'STAGING_FAILED', 502)
    }
...
router.post('/:jobId/complete', async (req, res, next) => {
  try {
    ...
    const originalById = new Map((job.candidates || []).map((candidate) => [
      candidate.candidateId,
      candidate
    ]))
    const reviewed = Array.isArray(req.body?.candidates) ? req.body.candidates : []
    const fields = []
    const seenCandidateIds = new Set()
    for (const item of reviewed) {
      if (!item || item.included !== true) continue
      const candidateId = String(item.candidateId || '')
      if (!candidateId || seenCandidateIds.has(candidateId)) continue
      seenCandidateIds.add(candidateId)
      const original = originalById.get(candidateId)
      if (!original) continue
      const merged = {
        ...(typeof original.field?.toObject === 'function' ? original.field.toObject() : original.field),
        ...(item.field && typeof item.field === 'object' ? item.field : {})
      }
      const fieldError = documentFieldValidationError(merged)
      if (fieldError) {
        return sendError(res, fieldError, 'INVALID_REVIEWED_FIELD', 400)
      }
      const field = sanitizeAiFields(
        [merged],
