...
function sanitizeDocumentDraft(
  ...
  for (const sourceField of rawFields) {
    ...
    const necessity = allowedNecessity.has(asStr(sourceField.necessity, 20).toLowerCase())
      ...
      : 'optional'
    if (necessity === 'exclude' || role === 'metadata') continue
    ...
    if (!validIds.length || requestedIds.length !== validIds.length) continue
    ...
    // Confidence cannot turn invented content into a real form control.
    if (!validIds.length || labelScore < 75 || (titleKey && labelKey === titleKey)) continue
    ...
    const generatorConfidence = clamp(
      sourceField.generatorConfidence ?? sourceField.mappingConfidence
    )
    const criticStatus = sourceField._criticStatus === 'unavailable' || raw?.criticStatus === 'unavailable'
      ? 'unavailable'
      : 'validated'
    const criticConfidence = criticStatus === 'validated'
      ? clamp(sourceField.criticConfidence)
      : 0
  ...
  for (const candidate of candidates) {
    const key = normalizeText(candidate.field?.label)
  ...
  return {
    ...
    confidenceSummary,
    detectedFieldCount: rawFields.length,
    truncated: structuredCandidates.length > MAX_FIELDS
  }
}
...
async function generateDocumentFormDraft(
  ...
  if (!result.candidates.length) {
    const error = new Error('No source-grounded form fields were generated')
    error.code = 'NO_FIELDS_DETECTED'
    throw error
  }
  return result
