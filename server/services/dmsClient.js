...
const connectionConfig = ({ apiKey, jwtToken, orgSlug } = {}) => ({
  apiKey: String(apiKey || '').trim() || String(process.env.DMS_API_KEY || '').trim(),
  jwtToken: String(jwtToken || '').trim() || String(process.env.DMS_JWT || '').trim(),
  rootUrl: baseUrl(),
...
async function testConnection({ apiKey, jwtToken, orgSlug, timeoutMs = 10000 } = {}) {
  if (!isEnabled()) {
    throw new DmsError('DMS is disabled on this server', { status: 503, code: 'DMS_DISABLED' })
  }

  const effective = connectionConfig({ apiKey, jwtToken, orgSlug })
  ...
  await dmsFetch('/documents?limit=1&offset=0', {
